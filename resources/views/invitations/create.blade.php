                    ...
                    <form action="{{ route('invitations.store') }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach($templates as $template)
                                    <div class="relative">
                                        <input type="radio" name="template_id" value="{{ $template->id }}" id="template_{{ $template->id }}" class="sr-only" {{ old('template_id') == $template->id ? 'checked' : '' }}>
                                        <label for="template_{{ $template->id }}" class="block cursor-pointer">
                                            <div class="border-2 border-gray-200 rounded-lg overflow-hidden hover:border-blue-500 transition-colors template-card">
                                                @if($template->preview_image)
                                                    <img src="{{ asset('storage/' . $template->preview_image) }}" alt="{{ $template->name }}" class="w-full h-32 object-cover">
                                                @else
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="wedding_time" id="wedding_time" value="{{ old('wedding_time') }}" 
                            ...
                            <div>
                                ...
                                <textarea name="location" id="location" rows="2" 
                                          ...
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('location') }}</textarea>
                                @error('location')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="additional_notes" id="additional_notes" rows="3" 
                                      ...
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('additional_notes') }}</textarea>
